...
<template>
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        ...
        <Modal :show="eventStore.showModal" @close="eventStore.handleCloseModal" class="max-w-xl">
            <h2 class="mb-5 truncate leading-tight font-semibold">{{ eventStore.form.id ? 'Form Edit User' : 'Form Tambah User' }}</h2>
            <form enctype="multipart/form-data" class="space-y-3" @submit.prevent="eventStore.handleSave">
                ...
                <Input id="title" type="text" required autofocus :tabindex="2" autocomplete="title"
                    v-model="eventStore.form.title" placeholder="Masukkan Judul" />
                <InputError :message="eventStore.error?.title?.[0]" />

                <Label for="description">Deskripsi</Label>
                <Textarea class="w-full" id="description" required :tabindex="2" autocomplete="description"
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div class="space-y-3">
                        ...
                        <Input id="start_time" type="datetime-local" required autofocus :tabindex="4"
                            autocomplete="start_time" v-model="eventStore.form.start_time" />
                        <InputError :message="eventStore.error?.start_time?.[0]" />
                    </div>

                    <div class="space-y-3">
                        <Label for="end_time">Selesai</Label>
                        <Input id="end_time" type="datetime-local" required autofocus :tabindex="4"
                            autocomplete="end_time" v-model="eventStore.form.end_time" />
                        <InputError :message="eventStore.error?.end_time?.[0]" />
                    </div>

                    <div class="space-y-3">
                        <Label for="registration_deadline">Batas Registrasi</Label>
                        <Input id="registration_deadline" type="datetime-local" required autofocus :tabindex="4"
                    ...
                    <div class="space-y-3">
                        ...
                        <Input id="preliminary_date" type="datetime-local" required autofocus :tabindex="4"
                    ...
                    <div class="space-y-3">
                        ...
                        <Input id="final_date" type="datetime-local" required autofocus :tabindex="4" autocomplete="final_date"
                    ...
                    <div class="space-y-3">
                        ...
                        <Select id="is_online" v-model="eventStore.form.is_online" :tabindex="5"
                            :options="eventStore.isOnlineOptions" placeholder="Pilih role" class="mt-2" />
                        <InputError :message="eventStore.error?.is_online?.[0]" />
                    </div>
                </div>
                
                <Label for="whatsapp_group_link">Whatsapp Group</Label>
                <Input id="whatsapp_group_link" type="text" required autofocus :tabindex="2"
                    autocomplete="whatsapp_group_link" v-model="eventStore.form.whatsapp_group_link"
                    placeholder="Masukkan Judul" />
                <InputError :message="eventStore.error?.whatsapp_group_link?.[0]" />

                <Label for="guidebook_link">Whatsapp Group</Label>
                <Input id="guidebook_link" type="text" required autofocus :tabindex="2" autocomplete="guidebook_link"
                ...
                <Textarea class="w-full" id="location" required :tabindex="2" autocomplete="location"
                ...
                <Textarea class="w-full" id="link_zoom" required :tabindex="2" autocomplete="link_zoom"
                ...
                <Input id="quota" type="number" required autofocus :tabindex="4" autocomplete="quota"
                    v-model="eventStore.form.quota" />
                <InputError :message="eventStore.error?.quota?.[0]" />

                <Button type="submit" class="mt-2 w-full" tabindex="6" :disabled="eventStore.isLoading">
