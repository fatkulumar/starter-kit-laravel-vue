...
import { TaskItem } from "@/stores/student/purchaseStore";
...
const allFilesUploaded = computed(() => {
  return props.tasks.every(task => task.file !== null);
});
...
<template>
  <div>
    ...
    <div class="grid grid-cols-2 gap-4">
      <div v-for="(task, index) in tasks" :key="index" class="relative w-full h-[160px]">
        <label
          ...
          <input type="file" accept="image/*" class="hidden" @change="(e) => onFileChange(e, index)" />
          ...
          <template v-else>
            <div class="w-full h-full overflow-auto hide-scrollbar">
              <img :src="task.preview" class="object-contain min-w-full min-h-full" />
            </div>
          </template>
        </label>

        <button v-if="task.preview" type="button" @click="onRemoveFile(index)"
